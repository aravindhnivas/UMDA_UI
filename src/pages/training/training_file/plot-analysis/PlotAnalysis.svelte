<script lang="ts">
    ...
    let filtered_dir = 'default';
    let dir_items_for_plotting: string[] = ['default'];

    const fetch_analysis_dir = async () => {
        const analysis_dir = await $post_analysis_files_directory;
        const filtered_dir = await path.join(analysis_dir, 'filtered');
        const dirs = await fs.readDir(filtered_dir);
        if (isEmpty(dirs)) {
            toast.error(`No directories found in ${filtered_dir}`);
            return;
        }
